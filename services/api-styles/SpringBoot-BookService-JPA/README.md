...
The `id` is generated by the database (`@GeneratedValue`), so it must
not be supplied in the request body; any `id` sent by the client is
ignored. On success the service responds with `201 Created`.

```Bash
$ curl -i -X POST http://localhost:8080/api/books  -H "Content-Type: application/json" -d '{"author":"Joshua Bloch","title":"Effective Java","isbn":"978-0134685991"}'
$ curl -i -X POST http://localhost:8080/api/books  -H "Content-Type: application/json" -d '{"author":"Robert C. Martin","title":"Clean Code","isbn":"978-0132350884"}'
$ curl -i -X POST http://localhost:8080/api/books  -H "Content-Type: application/json" -d '{"author":"Martin Fowler","title":"Refactoring","isbn":"978-0134757599"}'
```
