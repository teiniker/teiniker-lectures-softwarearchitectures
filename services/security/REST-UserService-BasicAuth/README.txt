...
URL: https://localhost:8443/REST-UserService-BasicAuth/v1/users

Accession the service via HTTP results in a redirect:
$ curl -i http://localhost:8080/REST-UserService-BasicAuth/v1/users
HTTP/1.1 302 Found
Connection: keep-alive
Location: https://localhost:8443/REST-UserService-BasicAuth/v1/users
Content-Length: 0
Date: Thu, 17 Dec 2020 17:51:37 GMT

Acessing the service without authentication results in a unauthorized error:
$ curl -i -k https://localhost:8443/REST-UserService-BasicAuth/v1/users
HTTP/2 401
expires: 0
...
For BASIC authentication, we have to provide username and password:
$ curl -i -k --user student:student https://localhost:8443/REST-UserService-BasicAuth/v1/users
HTTP/2 200
expires: 0
cache-control: no-cache, no-store, must-revalidate
pragma: no-cache
...
Example: 
	...
	HTTP Request:
		GET /REST-UserService-BasicAuth/v1/users/3 HTTP/1.1
		Authorization: Basic c3R1ZGVudDpzdHVkZW50
